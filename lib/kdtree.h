...
class KDTree
{
  ...
  public:
    ...
    void quickSelect(vector<Point<Dim>>& newPoints,int left, int right, int index, int dim); //quick Select to sort/find the median

    int partition(vector<Point<Dim>>& newPoints,int left, int right, int index, int dim); // helper fn for quick select

    void dimensionSort(vector<Point<Dim>>& newPoints,int left, int right, int index, int dim); // sorts the entire vector by its dimension constraint

    void destroyTree(KDTreeNode* subtree); // destructor for the whole tree

    KDTreeNode* copyTree(KDTreeNode* subtree); // copy constructor helper for creatign copied trees
    
    KDTreeNode* createNode(vector<Point<Dim>>& newPoints, int left, int right); // constructor helper fn for creating nodes/tree
    
    Point<Dim> findSmallest(KDTreeNode* subtree, const Point<Dim>& query, int dimen) const;// helper fn for findNearest Neighbor
    
    bool checkHyperRect(const Point<Dim>& target, Point<Dim>& currentBest, Point<Dim>& split, int dimen) const;
